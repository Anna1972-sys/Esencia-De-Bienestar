...
export function macrosFromSpecialist(result: MacroSpecialistResult) {
  return {
    ...
    total_calories: Number(result.totals?.kcal) || 0,
    total_protein: Number(result.totals?.protein) || 0,
    total_carbs: Number(result.totals?.carbs) || 0,
    total_fat: Number(result.totals?.fat) || 0,
    total_fiber: Number(result.totals?.fiber) || 0,
