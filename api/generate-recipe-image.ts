...
function buildFoodPhotoPrompt(title: string, ingredients: string[], preparation: string[] = [], retryReason = "") {
  ...
  return [
    ...
    "Respect realistic ingredient hierarchy: the main ingredient from the recipe title/preparation must visually dominate the dish; secondary ingredients must appear in natural smaller proportions, not with equal prominence.",
    "Do not fill the plate with a secondary ingredient. For example, peas must appear only as a modest accent unless peas are clearly the main ingredient of the recipe.",
    "The background must be clean white, bright and minimalist, but never overexposed, never empty, never almost completely white.",
    "Lighting: soft natural daylight as if from a side window, natural contrast, diffused highlights and realistic soft-angled shadows.",
    "Colors: vivid but realistic, clearly showing the real ingredients and the finished dish.",
    "Textures must be very detailed and realistic: meat with visible fibers, juices and a real caramelized sear from the maillard reaction; fresh vegetables with irregular shapes; glossy browned mushrooms when present; natural velvety sauce; no artificial or deformed food.",
    ...
    "The food must look cooked by a real person, with small natural imperfections. Do not arrange ingredients symmetrically or repetitively.",
    "Pieces must have varied sizes, positions and textures. Mushrooms, when present, must not repeat the same orientation or identical shape. Onion, when present, should be integrated naturally into the sauce or sauté.",
    "Small round ingredients such as peas must look tender and naturally varied, with irregular placement and modest quantity unless they are the main ingredient.",
    ...
    "Composition: clean, elegant, balanced and appetizing, optimized for a mobile wellness recipe app, with a subtle 50mm food-photography feel and shallow depth of field while keeping the main food sharp.",
